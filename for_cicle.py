...
# Мы также можем изменить элементы списка внутри коллекции (не создавая новую)
   ...
   for line in lines:
    ...
    # Также если необходимо изменить состав исходного списка, то обходим его копию.
    
    a = ['a', 'b', 'c']

    for i in a[:]:      # Можно использовать a.copy()
        a.pop(0)

    # Либо создать временный список, а потом очистить исходный и добавить элементы из временного
